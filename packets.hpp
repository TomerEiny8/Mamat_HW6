 ...
 *
 * This file is integral to the NIC simulation and can be extended with concrete
 * implementations and additional packet types as needed.
 */
...
class generic_packet {
     ...
     *        stored in. In the case of an l2 packet - the function will store
    ...
    /**
     * @fn full_packet_string
     * @brief Convert the packet to full string.
     *
     * @param [out] packet - Packet as a string.
     *
     * @return true in success, false in failure (e.g memory allocation failed).
     */
    virtual bool full_packet_string(std::string &packet) = 0;
